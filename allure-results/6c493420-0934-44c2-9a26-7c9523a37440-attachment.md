...
- Location: tests\loginPageTest.spec.js:8:5
...
> 15 |     await expect(loggedInUserName).toContainText('Welcome nimisha18')
     |                                    ^ Error: toContainText can be only used with Locator object
  16 | })
  17 | 
  18 | //Testcase-4:Verify login with invalid username and valid password
  19 | 
  20 | test('TC#4:Invalid username and Valid password', async ({ page }) => {
  21 | 
  22 |     const loginPage1 = new LoginPage(page)
  23 |     await loginPage1.gotoLoginPage()
  24 |     await loginPage1.loginData(testdatacredentials[0].username, testdatacredentials[0].password)
  25 |     await loginPage1.verifyInvalidLogin()
  26 |     await expect(loginPage.errorMessage).toContainText('User does not exist');
  27 | 
  28 | })
  29 | 
  30 | //Testcase-5: Verify login with valid username and invalid password
  31 | test('Valid username and Invalid password', async ({ page }) => {
  32 | 
  33 |     const loginPage2 = new LoginPage(page)
  34 |     await loginPage2.gotoLoginPage()
  35 |     await loginPage2.loginData(testdatacredentials[1].username, testdatacredentials[1].password)
  36 |     await loginPage2.verifyInvalidLogin()
  37 |     await expect(loginPage.errorMessage).toContainText('User does not exist');
  38 | 
  39 | })
  40 | 
  41 | //Testcase-6: Verify login with Invalid username and Invalid password
  42 | test('TC#6:Invalid username and Invalid password', async ({ page }) => {
  43 | 
  44 |     const loginPage3 = new LoginPage(page)
  45 |     await loginPage3.gotoLoginPage()
  46 |     await loginPage3.loginData(testdatacredentials[2].username, testdatacredentials[2].password)
  47 |     await loginPage3.verifyInvalidLogin()
  48 |     await expect(loginPage.errorMessage).toContainText('User does not exist');
  49 | 
  50 | })
```