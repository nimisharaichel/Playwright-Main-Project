...
- Name: loginPageTest.spec.js >> TC#4:Invalid username and Valid password
- Location: tests\loginPageTest.spec.js:20:5

# Error details

```
ReferenceError: Invalidusername is not defined
```
...
```ts
  ...
  2  |     constructor (page){
  3  |         this.page=page
  4  |         this.loginclickpbutton=page.locator('#login2')
  5  |         this.usernameInput = page.locator('#loginusername')
  6  |         this.passwordInput = page.locator('#loginpassword')
  7  |         this.loginButton = page.locator("//button[text()='Log in']")
  8  |         this.userNameLoggedIn = page.locator('#nameofuser')
  9  |         this.errorMessage = page.locator('text=User does not exist')
  10 |         this.okButton = page.getByRole('button', { name: 'OK' })
  11 |     }
  12 | 
  ...
  16 | 
  17 | 
  18 |      async loginData(validusername, validpassword) {
  19 |        await this.loginclickpbutton.click()
  20 |        await this.usernameInput.fill(validusername)
  21 |        await this.passwordInput.fill(validpassword)
  22 |         await this.loginButton.click()
  23 |     }
  24 | 
  25 |     async verifySuccessfulLogin() {
  26 | 
  27 |         
  28 |         await this.userNameLoggedIn.waitFor()
  29 |         const loggedInUserName = await this.userNameLoggedIn.textContent()
  30 |         return loggedInUserName
  31 |     }
  32 | 
  33 |     async verifyInvalidUsernameValidPassword(Invalidusername1,validpassword1)
  34 | {
  35 |     await this.loginclickpbutton.click()
> 36 |     await this.usernameInput.fill(Invalidusername)
     |                                   ^ ReferenceError: Invalidusername is not defined
  37 |     await this.passwordInput.fill(validpassword1)
  38 |     await this.loginButton.click()
  39 |     
  40 | }
  41 |  async verifyvalidUsernameInvalidPassword(validusername2,Invalidpassword2)
  42 | {
  43 |     await this.loginclickpbutton.click()
  44 |     await this.usernameInput.fill(Invalidusername2)
  45 |     await this.passwordInput.fill(validpassword2)
  46 |     await this.loginButton.click()
  47 |     
  48 | }
  49 | async verifyInvalidUsernameInvalidPassword(Invalidusername3,Invalidpassword3)
  50 | {
  51 |     await this.loginclickpbutton.click()
  52 |     await this.usernameInput.fill(Invalidusername3)
  53 |     await this.passwordInput.fill(Invalidpassword3)
  54 |     await this.loginButton.click()
  55 | 
  56 | }
  57 |     
  58 |  async verifyInvalidLogin() {
  59 | 
  60 |     this.page.once('dialog', async dialog => {
  61 | 
  62 |         const errormessage = dialog.message()
  63 | 
  64 |         console.log(errormessage)
  65 | 
