...
```ts
  ...
  3  | import testdatacredentials from '../Utils/testdatacredentials.json'
  4  | 
  ...
  10 | 
  11 |     const user = testdatacredentials[4].username
  12 |     const pass = testdatacredentials[4].password
  13 | 
> 14 |     const dialogPromise = page.waitForEvent('dialog')
     |                                ^ Error: page.waitForEvent: Test ended.
  15 |     await signupPage.signup(user, pass)
  16 | 
  17 |     const dialog = await dialogPromise
  18 | 
  19 |     expect(dialog.message()).toBe('Sign up successful.')
  20 |     await dialog.accept()
  21 | })
  22 | 
  23 | //Testcase-2:Close the sign up screen by clicking close button
  24 | 
  25 | test('Close the sign up screen', async({page})=>{
  26 |     const signuppage1=new SignupPage(page)
  27 |     await signuppage1.goto()
  28 |     await signuppage1.close()
  29 | 
  30 | })
  31 | 
```